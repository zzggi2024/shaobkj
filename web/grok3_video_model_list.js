...
const NODE_NAME = "Shaobkj_Grok3_Video";
...
const DEFAULT_MODELS = ["grok-video-3", "grok-video-3-10s"];
...
function setModelOptions(node, models, selectedValue) {
	...
	const selected = selectedValue || options[0] || "grok-video-3";

	if (selectWidget) {
		updateComboValues(selectWidget, options);
...
async function fetchModels(node, buttonWidget) {
	...
	if (!modelWidget) {
		...
		console.warn("[Shaobkj-Grok3] 未找到模型控件");
		return;
	}
	if (!apiKey || !apiBase) {
		showToast("请到后台查看具体错误", true);
		console.warn("[Shaobkj-Grok3] API密钥或API地址为空", { apiKeySet: Boolean(apiKey), apiBase });
		return;
	}
	...
	try {
		const response = await api.fetchApi("/shaobkj/grok3_video/models", {
			method: "POST",
		...
		showToast("请到后台查看具体错误", true);
		console.warn("[Shaobkj-Grok3]", error);
	} finally {
...
function setupModelFetcher(node) {
	...
	const modelWidget = findWidget(node, MODEL_WIDGET);
	if (!proxyWidget || !modelWidget || node.__shaobkjGrok3VideoModelFetcherReady) {
		return;
	}
	node.__shaobkjGrok3VideoModelFetcherReady = true;
	...
	setModelOptions(node, DEFAULT_MODELS, modelWidget.value || "grok-video-3");
	...
	const selectWidget = node.addWidget("combo", MODEL_SELECT_WIDGET, modelWidget.value || DEFAULT_MODELS[0], (value) => {
		setModelOptions(node, findWidget(node, MODEL_SELECT_WIDGET)?.options?.values || DEFAULT_MODELS, value);
...
app.registerExtension({
	name: "Shaobkj.Grok3VideoModelList",

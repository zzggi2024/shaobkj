...
class Shaobkj_Grok_Video:
    ...
    @staticmethod
    def _submit_with_curl(url, headers, payload, timeout, proxies=None, verify=False):
        curl_path = shutil.which("curl")
        total_timeout = timeout[1] if isinstance(timeout, (tuple, list)) and len(timeout) > 1 else timeout
        max_time = 86400 if total_timeout is None else max(1, int(total_timeout))
        connect_timeout = timeout[0] if isinstance(timeout, (tuple, list)) and len(timeout) > 0 else 20
        ...
        try:
            ...
            cmd.extend(["--data-binary", f"@{tmp_payload}", url])
            cmd = [part for part in cmd if part]

            print("[Shaobkj-Grok] submit via curl")
            ...
            status_text = result.stdout.decode("utf-8", errors="ignore").strip() or "0"
            stderr_text = result.stderr.decode("utf-8", errors="ignore").strip()
            try:
                status_code = int(status_text[-3:])
            except Exception:
                status_code = 0
            with open(tmp_response, "r", encoding="utf-8", errors="ignore") as f:
                response_text = f.read()
            if result.returncode != 0:
                raise RuntimeError(f"curl submit failed rc={result.returncode}, status={status_code}, stderr={stderr_text[:1000]}")
            return _SimpleResponse(status_code, response_text)
        finally:
            for path in (tmp_payload, tmp_response):
